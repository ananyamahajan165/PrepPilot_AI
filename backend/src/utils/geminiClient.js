 ...
 *
 * Key rotation: reads GEMINI_API_KEY_1..GEMINI_API_KEY_5 (skipping any that
 * aren't set), round-robins the *starting* key across separate requests so
 * load spreads evenly, and on a rate-limit/auth failure, retries the same
 * request against the next available key before giving up. Raising the
 * limit past 5 keys later is a one-line change (see MAX_KEY_SLOTS below) —
 * no other code needs to change.
 */

const MAX_KEY_SLOTS = 5; // bump this (and add more GEMINI_API_KEY_N vars) to scale past 5 keys

const API_KEYS = Array.from({ length: MAX_KEY_SLOTS }, (_, i) => process.env[`GEMINI_API_KEY_${i + 1}`])
...
if (API_KEYS.length === 0) {
  ...
  console.error(
    "[gemini] No Gemini API keys found. Set at least GEMINI_API_KEY_1 (and optionally _2 through _5) in your .env."
  );
}
